...
<?php
	...
	$query0="SELECT * FROM `agahi` WHERE `group` = ? and `city`= ? and `title` LIKE  ?";
	$result0=$connect->prepare($query0);
	$result0->bindValue(1,$_GET["group"]);
	$result0->bindValue(2,$_GET["city"]);
	$result0->bindValue(3,'%'.$_GET["searchtxt"].'%');
	$result0->execute();
	$tedad0=$result0->rowCount();
	
	
	
	$query="SELECT * FROM `agahi` WHERE `group` = ? and `city`= ? and `title` LIKE  ? limit ".$_GET["page"]." , 3";
	$result=$connect->prepare($query);
	$result->bindValue(1,$_GET["group"]);
	$result->bindValue(2,$_GET["city"]);
	$result->bindValue(3,'%'.$_GET["searchtxt"].'%');
	$result->execute();
...
for($i=0;$i<$number;$i++)
{
	$i2=$i+1;
	$startpagenumber=$i*3;
	echo '<a href=?page='.$startpagenumber.'&city='.$_GET["city"].'&group='.$_GET["group"].'&searchtxt='.$_GET["searchtxt"].'>'.$i2.'</a>&nbsp;&nbsp;&nbsp;&nbsp;';
}
