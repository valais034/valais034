...
<?php
	...
	$query0="SELECT * FROM `agahi` WHERE `city` = ? ";
	$result0=$connect->prepare($query0);
	$result0->bindValue(1,$_GET["ostan"]);
	$result0->execute();
	$tedad0=$result0->rowCount();
	
	
	
	$query="SELECT * FROM `agahi` WHERE `city` = ? limit ".$_GET["page"]." , 3";
...
for($i=0;$i<$number;$i++)
{
	$i2=$i+1;
	$startpagenumber=$i*3;
	echo '<a href=?page='.$startpagenumber.'&ostan='.$_GET["ostan"].'>'.$i2.'</a>&nbsp;&nbsp;&nbsp;&nbsp;';
}
